...
class SQLiteTraceStorage(TraceStorage):
    ...
    def query_spans(self, query: TraceQuery) -> list[TraceSpan]:
        ...
        # Build WHERE clause from hardcoded condition strings (no user input)
        # All values are parameterized through the params list
        if conditions:
        ...
        with self._cursor() as cursor:
            # Note: where_clause is built from hardcoded strings only, not user input
            # All actual values are passed via params (parameterized query)
            cursor.execute(
                "SELECT * FROM spans WHERE "  # nosec B608
                + where_clause
    ...
    def cleanup_old_traces(self, retention_days: int) -> int:
        ...
        with self._cursor() as cursor:
            ...
            if trace_ids:
                # Build parameterized IN clause - placeholders are just "?,?,?"
                # All actual values are passed via trace_ids (parameterized query)
                placeholders = ",".join("?" * len(trace_ids))
                cursor.execute(
                    "DELETE FROM spans WHERE trace_id IN (" + placeholders + ")",  # nosec B608
                    trace_ids,
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._cursor() as cursor:
            ...
            cursor.execute(
                "SELECT COUNT(DISTINCT agent_id) FROM spans WHERE agent_id IS NOT NULL"
            )
...
class SQLiteSpanExporter:
    ...
    def _convert_span(self, otel_span: Any) -> TraceSpan:
        ...
        return TraceSpan(
            ...
            name=otel_span.name,
            kind=otel_span.kind.name.lower()
            if hasattr(otel_span.kind, "name")
            else "internal",
